...
def batch_violation_rate(
    ...
    if not imagined_batch:
        return 0.0
    # Mean-per-step disagreement averaged over pairs:
    # R_H = (1/N) Σ_n (1/H) Σ_t 1[imag_t ≠ real_t]
    rates = [
        imagined_realized_disagreement(
            {'actions': np.asarray(img['actions'])[:horizon]},
            {'actions': np.asarray(rea['actions'])[:horizon]},
            kmeans,
        )
