    ...
    def calibrate(
        ...
        for sigma in self._sigma_grid:
            ...
            for _ in range(n_pairs):
                ...
                # FIX: env.reset(seed) drifts across calls; run realized first,

                # feed its true obs[0] to imagined for fair pairing.

                prev_tau = self.dreamer.get_actor_temperature()

                self.dreamer.set_actor_temperature(float(sigma))

                real_full = self.dreamer.rollout(

                    env, self.horizon, seed=pair_seed)

                self.dreamer.set_actor_temperature(prev_tau)

                obs_real0 = np.asarray(real_full['obs'][0])

                img = self.dreamer.rollout_imagined(

                    obs_real0, self.horizon, sigma=float(sigma), seed=pair_seed)

                rea = {'actions': real_full['actions']}
